...
int count=0;
    ...
    for (;;) {
        ...
        if (nready > 0) {
            ...
            //if (count>=200)
            // {
              //  if (write(serfd, buffer, byte_num/20) < 0) {
           ...
           // }
            ...
            if (FD_ISSET(serfd, &rset)) {
             ...
             /*                             if (count>=200)
                                            {
                                                if (rate_control(buffer, byte_num))
                                                {
                                                    if (write(clifd, TotalBuffer, TotalByte) < 0) {
                                                         printf("[x] Write to client failed.\n");
                                                         break;
                                                        }
                                                    memset(TotalBuffer,0,TotalByte);
                                                    TotalBuffer[0]='\0';
                                                    TotalByte=0;
                                                }
                                            }  
...
void rate_control(int rate){
    ...
    r = rate/2.0;
    while(1){
    if((float)(clock()-cur)>CLOCKS_PER_SEC/r)
    break;
}
